...
- `email-sync-improvements` (specced 2026-08-10) adds an Email Sync page (trigger + run history) to the nav and richer per-message data worth displaying here: display names, both timestamps, read/unread, importance, flags, categories, folder, attachment metadata, and an open-in-Outlook link. That page shows runs, not messages — the browsing surfaces are still this feature.
