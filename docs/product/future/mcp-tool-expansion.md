...
`mcp-server` shipped (auth plus the first tool set). Lane-targeting/move tools additionally depend on `move-task-between-lanes`; tag tools on a future tags feature (core-brief work, not yet specced or stubbed); further email tools on `email-sync`.
