...
work-helper is a self-hosted personal CRM. It's a TypeScript web app that
also exposes an MCP server (the "work-helper" MCP), so both Tyler and AI
agents can query and act on the same data.
...
- **Tags and custom fields** — flexible metadata across people, emails,
  and tasks.
...
- **MCP server:** built on the official `@modelcontextprotocol/sdk`. No
  other MCP framework.
- **Email ingestion:** the work-helper server pulls directly from
  Microsoft Graph (scheduled polling and/or webhook) — ingestion is a
  server-side concern, not something an AI agent does. AI agents interact
  with work-helper only as **consumers** of the work-helper MCP's tools
  (query people/emails/tasks, link records together, manage tags).
- **Deployment:** self-hosted, via Docker.

These constraints are binding. A feature spec that conflicts with one of
them needs the constraint changed first (in the constitution), not a
