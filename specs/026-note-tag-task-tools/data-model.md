...
| `deleteTag` detach-count variant | `src/server/services/tags.ts` | `{ ok: true; peopleDetached: number; tasksDetached: number } \| { ok: false; error: 'tag-not-found' }` (same variant name `resolveExistingTag` uses) |
