...
- [X] T036 Open the PR with a Conventional Commits title, calling out the single additive `tasks.archived` migration (`drizzle/0005_*.sql`, non-destructive `ALTER TABLE ADD`) and that `delete-card` and `board-search-filter`'s own tag-selector/persistence are unaffected, and let the Claude Code CI review run on the diff
