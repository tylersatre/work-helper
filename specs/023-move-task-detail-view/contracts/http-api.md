...
- Placement requests are serialized through a local promise chain (research D4), so concurrent clicks reach the server in click order and every response is applied as it settles — the last one to apply is by construction the last one clicked. On a failed result, the pill row keeps showing the task's actual last-saved lane and an inline error message is shown, following the existing `tagError`-style `role="alert"` precedent already in this page (research D3; spec edge case 2).
