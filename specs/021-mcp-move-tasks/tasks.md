...
- [ ] T008 [US2] Verify the T005 implementation in src/server/mcp/tools.ts satisfies T006/T007 (clamping via the service, `landedPosition` from the returned row, `z.number().int().min(1)` boundary rejection); fix any gaps until T006/T007 pass with T003/T004 still green
