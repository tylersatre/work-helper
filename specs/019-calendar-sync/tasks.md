...
- [ ] T006 Implement `FakeCalendarProvider` in `src/server/services/calendar/fake-provider.ts` — seedable from `SeedEvent[]`, reimplementing overlap-window and paging semantics with the same failure-injection knobs as `FakeMailProvider` — until T005's tests pass
