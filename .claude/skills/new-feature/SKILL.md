...
4. The project constitution (`.specify/memory/constitution.md`, or wherever `specify init` placed it) — the definition of done and constraints the criteria must respect
...
- The doc must describe the thinnest vertical slice that delivers the idea end to end. If the idea is bigger than one slice, propose a split (this slice now, named follow-ups later), confirm it with Tyler, and put everything else under Out of scope.
...
1. Write the file.
2. Show Tyler the full doc plus a two-line summary of the scope decisions (what's in, what got split out).
3. Revise per his feedback until he approves.
4. Stop. Do not run any speckit commands, do not write code, do not create branches. Remind him the next step is a fresh session from clean `main`, big model, `/speckit.specify @docs/product/features/<file>.md`.